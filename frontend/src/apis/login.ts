...
export const spotOnLogin = async (kakaoToken: string) => {
  try {
    const res = await axiosInstance.post("/user-service/login", {   // (기존) /user-service/login 2025.05.12
    ...
    console.log('result=======================>',res.status,res.data.accessToken,res.data.tokenId);  // log확인하기 추가 2025.05.08
...
export const join = async (joinInfo: JoinRequestParam) => {
  // console.log('joinInfo==>',joinInfo); // 2025.05.10 joinInfo확인하기
  ...
  // console.log('resData==>',res.data); // 2025.05.10 resData확인하기
  return res.data;
};
